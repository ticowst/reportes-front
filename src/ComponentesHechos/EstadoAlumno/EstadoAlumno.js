...
class EstadoAlumno extends Component {
    ...
    obtenerGrafica() {
        fetch(url+'estadoAlumnoGrafica?fecha_inicio='+this.state.anioini+'&fecha_fin='+this.state.aniofin)
        .then((response)=>{
    ...
    obtenerTabla(){
        ...
        fetch(url+'estadoAlumno?fecha_inicio='+this.state.anioini+'&fecha_fin='+this.state.aniofin)//hace el llamado al dominio que se le envió donde retornara respuesta de la funcion
        .then((response)=>{
            return response.json();
        })
        .then(async (result)=>{
