...
class PoblacionEstudiantil extends Component {
    ...
    obtenerGrafica() {
        fetch(url+'poblacionEstudiantil?fecha_inicio='+this.state.anioini+'&fecha_fin='+this.state.aniofin)
        .then((response)=>{
        ...
        .then(async (result)=>{
            ...
            var arregloDatos = [];
            var cadena1=this.state.anioini;

            for(var i in result){
                arregloDatos.push({y:parseInt(result[i]["count"]),label:cadena1});
                cadena1++;
            }
